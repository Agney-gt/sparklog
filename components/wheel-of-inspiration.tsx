  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-transparent">
      <div className="relative w-64 h-64 md:w-96 md:h-96 overflow-hidden">
        <div className="absolute top-0 left-1/2 w-1 h-8 bg-white -ml-0.5 z-10 overflow-hidden rounded-full"></div>
        <motion.div
          ref={wheelRef}
        ...
        >
          {segments.map((segment, index) => (
              <div
                key={index}
                className={`absolute top-0 left-0 w-1/2 h-1/2 origin-bottom-right overflow-hidden opacity-80 bg-gradient-to-r ${segment.color}`}
                style={{
