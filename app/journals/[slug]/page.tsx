...
// Helper function to load and parse a Markdown file from /content/journals
async function getJournalContent(slug: string) {
  const filePath = path.join(process.cwd(), "content", "journals", `${slug}.md`);
  try {
...
export default async function JournalPage({ params }: JournalPageProps) {
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 py-8">
      ...
      <ReactMarkdown className="prose prose-lg">{journal.content}</ReactMarkdown>
    </article>
  );
}
